...
<?php
      ...
      if(empty($result_check))
      {
       ...
       ob_start();
          ...
          {
          echo '<option value="' . $count[0] . '">' . $count[3] . '</option>';
          }
          echo '</select></p> <br>
          ...
          if (isset($_POST['button'])) {
              ...
              $sql_wordlist_1 = "SELECT arcw_word.*, arcw_word_list.name FROM arcw_word, ARCW_users, arcw_word_list WHERE user_id = ARCW_users.id AND arcw_word_list.id = $slovar AND arcw_word.list_id = $slovar AND ARCW_users.login = '$ses'";
          ...
          if (isset($_POST['button2'])) {
            ...
            if(array_key_exists($key_search, $massive) == true)
            {
              ...
              if(empty($massive))
              {
                ...
                echo '<a class="btn" href="https://www.arcaim.net/word_lernen_private" name="arca_AS">Вернуться в меню выбора игр</a> <br>';
                /*var_dump($os);
                var_dump($random_nummer);
                var_dump($max);
                var_dump($count_max);
