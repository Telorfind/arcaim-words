      ...
      $pas=$_SESSION['name'];
       ...
       ob_start();
        if ($stat === PGSQL_CONNECTION_OK) {
          $sql_QL = "SELECT arcw_word_list.* FROM arcw_word_list, arcw_users WHERE user_id = arcw_users.id AND LOWER(arcw_users.login) LIKE '%$ses%'";
          $result_QL = pg_query($link,$sql_QL);
          echo '<form action="" method="post">
          <p>Выберите словарь: <p><select name="limit">
          <option disabled>Выберите словарь</option>'; 
          while($count = pg_fetch_row($result_QL))
          {
          echo '<option value="' . $count[0] . '">' . $count[2] . '</option>';
          }
          echo '</select></p> <br>
          Игра: <p> <select name="game"> <br>
          <option disabled>Выберите игру</option>
          <option value="game1">Обычный режим</option> 
          <br>
          </select></p> <br>
          <input type="submit" name="button" value="Начать игру!"> <br>
          </form>';
          if (isset($_POST['button'])) {
              ...
              $sql_wordlist_1 = "SELECT arcw_word.*, arcw_word_list.name FROM arcw_word, arcw_users, arcw_word_list WHERE user_id = arcw_users.id AND arcw_word_list.id = $slovar AND arcw_word.list_id = $slovar AND LOWER(arcw_users.login) LIKE '%$ses%'";
          ...
          if (isset($_POST['button2'])) {
            ...
            echo "<button id='activate'>Произнести вслух</button>
            <div class='main'>
            <p>Слово
            <input id='text' value='$word_slovo'>
        ...
        if (isset($_POST['button3']))
        {
          ...
          ob_start();
          $translate_word = $_POST['users_antwort'];
          echo "Вы написали $translate_word";
          ...
          echo "Правильный ответ: $true_ant";
          if($translate_word == $true_ant)
          {
            echo "Ответ верный, переходите к следующему слову!";
            $key = $random_nummer + 1;
            unset($massive[$key]);
            ...
            echo "Ответ НЕ ВЕРНЫЙ! Запомните перевод этого слова и повторите попытку позже!";
            echo '<form action="" method="post">
            <input type="submit" name="button2" value="Перейти к следующему слову!"> <br>
            </form>';   
           }
